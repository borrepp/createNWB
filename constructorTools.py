    ...
    if nsFile is None:
        ...
        if analogAccl is not None:
            if analogAccl['exists']:
                ...
                analogAccl['nsAnalog'] = [
                    rippleTools.AnalogIOchannel(nsFile=nsFile, chanName='leftAccelerometer', 
                                                acclSensitivity=analogAccl['shakerInfo']['leftAcclSensitivity']),
                    rippleTools.AnalogIOchannel(nsFile=nsFile, chanName='rightAccelerometer', 
                                                acclSensitivity=analogAccl['shakerInfo']['rightAcclSensitivity'])
                ]
        
        # Get Analog MicrosStimulation
        microStimulation = {
            'microStim_ns': rippleTools.get_stimTimeStamps(nsFile), 
        ...
        if analogFix is not None:
            if analogFix['exists']:
                ...
                analogFix['nsAnalog'] = rippleTools.AnalogIOchannel(nsFile=nsFile, chanName='fixON')

        # Get Analog Visual Events Photodiode
        if analogVisualEvents is not None:
            if analogVisualEvents['exists']:
                analogVisualEvents['photodiodeDuration'] = photodiodeDuration
                analogVisualEvents['nsAnalog'] = rippleTools.AnalogIOchannel(nsFile=nsFile, chanName='visualON')

        if analogTemp is not None:
            if analogTemp['exists']:
                analogTemp['nsAnalog'] = []
                for themID in analogTemp['thermistorIDs']:
                    analogTemp['nsAnalog'].append(rippleTools.AnalogIOchannel(nsFile=nsFile, chanName=themID))
...
def getNWB_rawElectrodes(dictYAML, nsFile, verbose=True):
    ...
    electrodesNS = rippleTools.get_rawElectrodeInfo(nsFile)
    ...
    return electrodesDict, electrodeGroups, devices

###################################################################################################################

def create_rawElectrodeGroup_hdf5(electrodesDict, nsFile, groupID, verbose=True):
    ...
    temp_filePath = temp_TimeSeries_hdf5(nsFile, entityIndexes=entities_index, tempName=groupName, 
                itemCount = int(groupInfo['item_count']), verbose=verbose)

    return temp_filePath, groupInfo
